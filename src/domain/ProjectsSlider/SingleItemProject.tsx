...
export default function SingleItemProject({ dataProject }: ProjectsListItemSlide) {
    ...
    const category: String =
        (dataProject.project_categories.data.length > 0 &&
            dataProject.project_categories.data[0].attributes.title) ||
        '';
    return (
        <div className='flex flex-col gap-5 justify-between w-full h-full font-thin'>
            ...
            <div className='flex flex-col justify-between h-full gap-5'>
                <div className='w-full flex flex-wrap gap-3 items-center justify-center'>
                    <h2 className='font-normal text-center'>{dataProject.title}</h2>
                </div>

                <div className='w-full flex flex-wrap gap-3 items-center justify-center'>
                    {!!dataProject.desc && (
                        <p className='text-center'>
                            {dataProject.desc.substring(0, 150) +
                                (dataProject.desc.length > 150 ? ' ...' : '')}
                        </p>
                    )}
                </div>
                ...
                <div className='flex flex-wrap items-center justify-between gap-2'>
                    <div className='flex items-center gap-2'>
                        <span
                            className={`py-1 px-3 font-bold rounded-md text-xs ${
                                dataProject.developing
                        ...
                        {!!category && (
                            <span className='py-1 px-3 font-bold rounded-md text-xs bg-blue-50 text-blue-800'>
                                {category}
                            </span>
                        )}
                    </div>
